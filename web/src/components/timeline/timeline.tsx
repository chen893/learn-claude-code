...
export function Timeline() {
  ...
  const maxLoc = Math.max(...versions.map((item) => item.loc), 1);
    ...
    <div className="flex flex-col gap-12">
      ...
      <div className="relative">
        {LEARNING_PATH.map((versionId, index) => {
          ...
          const locPercent = Math.round((data.loc / maxLoc) * 100);
              ...
              <div className="flex flex-col items-center">
                ...
                {!isLast && (
                  <div
                    className={cn(
                      "w-0.5 flex-1",
                      LAYER_LINE_BG[VERSION_META[LEARNING_PATH[index + 1]]?.layer || meta.layer]
                    )}
                  />
                )}
              </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          {LEARNING_PATH.map((versionId) => {
            ...
            const widthPercent = Math.max(2, Math.round((data.loc / maxLoc) * 100));
              ...
              <div key={versionId} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="h-5 w-full overflow-hidden rounded bg-zinc-100 dark:bg-zinc-800">
                    ...
                    >
                      <span className="text-[10px] font-medium text-white">{data.loc}</span>
                    </motion.div>
                  </div>
                </div>
              </div>
